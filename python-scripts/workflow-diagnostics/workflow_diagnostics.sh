...
# Activate python-scripts virtual environment (required)
# SCRIPT_DIR is /path/to/python-scripts/workflow-diagnostics
# We want /path/to/python-scripts/venv
PYTHON_SCRIPTS_VENV="$(dirname "$SCRIPT_DIR")/venv"
if [ ! -f "$PYTHON_SCRIPTS_VENV/bin/activate" ]; then
    echo -e "${RED}Error: Python virtual environment not found at $PYTHON_SCRIPTS_VENV${NC}"
    echo -e "${YELLOW}Please run: cd python-scripts && python3 -m venv venv && source venv/bin/activate && pip install -r requirements.txt${NC}"
    exit 1
fi
...
# Now activate our specific venv
