...
aws s3api head-bucket --bucket $S3_BUCKET --no-cli-pager 2>/dev/null || handle_error "Bucket does not exist or you don't have permission to access it"
echo "✅ Bucket exists and is accessible"

# Test 2: Verify encryption configuration
echo -e "\n2. Testing encryption configuration..."
ENCRYPTION=$(aws s3api get-bucket-encryption --bucket $S3_BUCKET --no-cli-pager 2>/dev/null)
...
CORS=$(aws s3api get-bucket-cors --bucket $S3_BUCKET --no-cli-pager 2>/dev/null)
if [[ -z "$CORS" ]]; then
  handle_error "CORS configuration is missing"
fi
if [[ "$CORS" == *"localhost:5173"* ]] || [[ "$CORS" == *"localhost:3000"* ]]; then
...
LIFECYCLE=$(aws s3api get-bucket-lifecycle-configuration --bucket $S3_BUCKET --no-cli-pager 2>/dev/null)
if [[ -z "$LIFECYCLE" ]]; then
...
aws s3 cp $TEST_FILE s3://$S3_BUCKET/$TEST_KEY --no-cli-pager 2>/dev/null || handle_error "Failed to upload test file"

# Check if the file exists
echo "   - Verifying file exists..."
aws s3 ls s3://$S3_BUCKET/$TEST_KEY --no-cli-pager 2>/dev/null || handle_error "Uploaded file not found"

# Download the file
echo "   - Downloading test file..."
aws s3 cp s3://$S3_BUCKET/$TEST_KEY ${TEST_FILE}.downloaded --no-cli-pager 2>/dev/null || handle_error "Failed to download test file"
...
aws s3 rm s3://$S3_BUCKET/$TEST_KEY --no-cli-pager 2>/dev/null || handle_error "Failed to delete test file"
