...
Provides unified API endpoints for FZIP (Financial ZIP) import/export functionality:
- POST /fzip/export - Initiate new FZIP export
- GET /fzip/export/{jobId}/status - Check FZIP export status  
- GET /fzip/export/{jobId}/download - Download FZIP package
- DELETE /fzip/export/{jobId} - Delete FZIP export
- GET /fzip/export - List user's FZIP exports
- POST /fzip/import - Create new FZIP import job
- GET /fzip/import - List user's FZIP imports
- GET /fzip/import/{jobId}/status - Get FZIP import status
- DELETE /fzip/import/{jobId} - Delete FZIP import job
- POST /fzip/import/{jobId}/upload - Upload FZIP package and start import
"""
...
from models.fzip import (
    FZIPJob, FZIPExportRequest, FZIPImportRequest, FZIPResponse, FZIPStatusResponse,
    FZIPStatus, FZIPType, FZIPExportType, FZIPFormat, FZIPMergeStrategy,
...
def get_fzip_export_download_handler(event: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if export_job.status != FZIPStatus.EXPORT_COMPLETED:
...
def list_fzip_exports_handler(event: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        export_jobs, pagination_key = list_user_fzip_jobs(user_id, FZIPType.EXPORT.value, limit)
        ...
        for job in export_jobs:
            exports_data.append({
                ...
                "exportType": job.export_type.value if job.export_type else None,
...
def list_fzip_imports_handler(event: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get import jobs
        import_jobs, next_key = list_user_fzip_jobs(user_id, FZIPType.IMPORT.value, limit, last_evaluated_key)
...
def upload_fzip_package_handler(event: Dict[str, Any], user_id: str, job_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if import_job.status != FZIPStatus.IMPORT_UPLOADED:
            return {
                'statusCode': 400,
        ...
        fzip_service_instance.start_import(import_job, import_job.s3_key)
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if route == "POST /fzip/export":
            return initiate_fzip_export_handler(event, user_id)
        elif route == "GET /fzip/export":
            return list_fzip_exports_handler(event, user_id)
        elif route == "GET /fzip/export/{jobId}/status":
            return get_fzip_export_status_handler(event, user_id)
        elif route == "GET /fzip/export/{jobId}/download":
            return get_fzip_export_download_handler(event, user_id)
        elif route == "DELETE /fzip/export/{jobId}":
            return delete_fzip_export_handler(event, user_id)
        elif route == "POST /fzip/import":
            return create_fzip_import_handler(event, user_id)
        elif route == "GET /fzip/import":
            return list_fzip_imports_handler(event, user_id)
        elif route == "GET /fzip/import/{jobId}/status":
            job_id = event.get('pathParameters', {}).get('jobId')
            return get_fzip_import_status_handler(event, user_id, job_id)
        elif route == "DELETE /fzip/import/{jobId}":
            job_id = event.get('pathParameters', {}).get('jobId')
            return delete_fzip_import_handler(event, user_id, job_id)
