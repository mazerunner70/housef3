...
def create_file_map_handler(event: Dict[str, Any], user: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not user_id_str:
            logger.error("User ID not found in authenticated user details.")
            return create_response(401, {"error": "Unauthorized", "message": "User ID missing."})
        ...
        new_file_map_instance = FileMap(
            userId=user_id_str, # Pass user_id directly
...
def update_file_map_handler(event: Dict[str, Any], user: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if update_data_dto:
            ...
            if changed:
                file_map.updated_at = datetime.now(timezone.utc) # Manually update timestamp
