    ...
    const fetchTransactionFiles = useCallback(async () => {
        ...
        try {
            ...
            const transactionFiles: TransactionFileDisplay[] = response
                .map((file: { fileId: string; fileName: string; uploadDate: string | number; startDate?: number; endDate?: number; transactionCount?: number; processingStatus?: string }) => ({
                    fileId: file.fileId,
            ...
            setError(err instanceof Error ? err.message : 'Failed to load transaction files');
        } finally {
            setLoading(false);
        }
    }, [accountId]);
